...
export default function Emails() {
  ...
  useEffect(() => {
    if (!composer.open || sending || sendingRef.current) return
  ...
  const sendEmail = async () => {
    if (!canCreate || sending || sendingRef.current) return
      ...
      {composer.open && (
        ...
        >
          <form
            ...
            className="flex max-h-[100dvh] min-h-0 w-full max-w-3xl flex-col overflow-hidden bg-slate-900 shadow-2xl sm:max-h-[calc(100dvh-2rem)] sm:rounded-2xl sm:border sm:border-white/10"
          >
